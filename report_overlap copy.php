...
foreach ($rows as $r) {
    ...
    if (($r['overlap_type'] ?? 'ไม่กันเหลื่อม') === 'กันเหลื่อม') {
        $totalOverlap += $amt;
    } else {
        $totalNormal += $amt;
    }
...
if (isset($_GET['export']) && $_GET['export'] === 'excel') {
    ...
    <table style="margin-bottom:12px;">
        <tr>
            <td colspan="4" class="title">รายงานงวดงาน / งบกันเหลื่อม</td>
        </tr>
        <tr>
            <td><strong>ปีงบประมาณ</strong></td>
            <td><?= h($selected_year ?: 'ทั้งหมด') ?></td>
            <td><strong>ประเภทงบ</strong></td>
        ...
        <tr>
            ...
            <td class="text-right"><?= number_format($totalOverlap, 2) ?></td>
        </tr>
        <tr>
            <td><strong>งบไม่กันเหลื่อม</strong></td>
            <td class="text-right"><?= number_format($totalNormal, 2) ?></td>
    ...
    <div class="card filter-card mb-4">
        ...
        <div class="card-body px-4 pb-4">
            <form method="get" class="row g-3">
                ...
                <div class="col-md-3">
                    <label class="form-label fw-semibold">ประเภทงบ</label>
...
<div>โครงการทั้งหมด: <b><?= $s['project_total'] ?></b></div>
<div>โครงการกันเหลื่อม: <b class="text-danger"><?= $s['project_overlap'] ?></b></div>

<hr>

<div>งบกันเหลื่อม: <b class="text-danger"><?= number_format($s['overlap_amount'],2) ?></b></div>
<div>งบทั้งหมด: <b><?= number_format($s['total_amount'],2) ?></b></div>
                ...
                <table class="table table-bordered table-hover align-middle" id="reportTable">
                    ...
                    <tfoot class="table-light">
                        <tr>
                            ...
                            <th class="number text-primary"><?= number_format($totalAmount, 2) ?></th>
                            <th colspan="4"></th>
                        </tr>
                    </tfoot>
                    <?php endif; ?>
