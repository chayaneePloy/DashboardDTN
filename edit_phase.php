...
if (!$phase_id || !ctype_digit((string)$phase_id)) {
    ...
    exit("ไม่ระบุงวดงาน (phase_id) ให้ถูกต้อง");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete') {
        ...
        if (!is_numeric($amountInput)) {
            ...
            if ($amount < 0) {
                $errorMsg = "จำนวนเงินต้องเป็นค่าบวก";
            }
        }
        ...
        if (!in_array($overlap_type, $allowedOverlap, true)) {
            $overlap_type = 'ไม่กันเหลื่อม';
        }

        if (!$errorMsg && $phase_number <= 0) {
            $errorMsg = "กรุณาระบุงวดที่ให้ถูกต้อง";
        }

        // due_date <= completion_date
        if (!$errorMsg && $due_date && $completion_date && ($due_date > $completion_date)) {
            $errorMsg = "วันที่เริ่มงวดงานต้องไม่เกินวันที่สิ้นสุดงวดงาน";
        }

        // กันเลขงวดซ้ำภายใต้สัญญาเดียวกัน (ยกเว้น phase ตัวเอง)
        if (!$errorMsg) {
            $stmt = $pdo->prepare("
                SELECT COUNT(*)
                FROM phases
                WHERE contract_detail_id = (
                    SELECT contract_detail_id FROM phases WHERE phase_id = ?
                )
                AND phase_number = ?
                AND phase_id <> ?
            ");
            $stmt->execute([$phase_id, $phase_number, $phase_id]);

            if ($stmt->fetchColumn() > 0) {
                $errorMsg = "เลขงวด {$phase_number} ซ้ำในสัญญาเดียวกัน";
            }
        }
...
if ($saveSuccess && $phase) {
    $redirectUrl = 'dashboard_report.php?' . http_build_query([
        ...
        'overlap_type' => $phase['overlap_type'] ?? '',
    ]);
}
...
<nav class="navbar navbar-expand-lg navbar-dark bg-primary">
    <div class="container">
        <a class="navbar-brand fw-bold" href="index.php">📊 Dashboard การจ่ายงวด</a>
            ...
            <form method="post" class="row g-3">
                ...
                <div class="col-md-3">
                    ...
                    <select name="overlap_type" class="form-select">
                        <?php foreach ($allowedOverlap as $ov): ?>
                            <option value="<?= h($ov) ?>" <?= ($ov === ($phase['overlap_type'] ?? 'ไม่กันเหลื่อม')) ? 'selected' : '' ?>>
                                <?= h($ov) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                    <div class="form-text text-danger">
                        *หากงวดนี้เป็นงบกันเหลื่อม ให้เลือก "กันเหลื่อม"
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="due_date" class="form-control"
                        value="<?= h(toThaiDisplay($phase['due_date'])) ?>" placeholder="dd/mm/yyyy">
                </div>

                <div class="col-md-4">
                    <label class="form-label">วันที่สิ้นสุดงวดงาน</label>
                    <input type="text" name="completion_date" class="form-control"
                        value="<?= h(toThaiDisplay($phase['completion_date'])) ?>" placeholder="dd/mm/yyyy">
                </div>

                <div class="col-md-4">
                    <label class="form-label">วันที่จ่ายเงินงวดงาน</label>
                    <input type="text" name="payment_date" class="form-control"
                        value="<?= h(toThaiDisplay($phase['payment_date'])) ?>" placeholder="dd/mm/yyyy">
                <div class="form-text text-danger">
                        *ใส่วันที่ออกหนังสือจาก ศจ.
                    </div>
                    </div>

                <div class="col-md-12">
                    <label class="form-label">รายละเอียด</label>
                    <textarea name="phase_name" class="form-control" rows="3"><?= h($phase['phase_name']) ?></textarea>
                </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-footer justify-content-center">
                <button type="button" class="btn btn-success" id="btnSaveOk">ตกลง</button>
            </div>
        </div>
    </div>
</div>
